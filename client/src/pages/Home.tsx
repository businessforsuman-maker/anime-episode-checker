...
export default function Home() {
  ...
  const fetchListQuery = trpc.anime.fetchList.useQuery();
  ...
  const handleFetchList = async () => {
    try {
      const data = await fetchListQuery.refetch();
      if (data.data && data.data.files) {
        setAnimeList(data.data.files);
        setCurrentItemIndex(Math.max(0, startItemNumber - 1));
      }
    } catch (error) {
      alert('Error fetching anime list: ' + (error instanceof Error ? error.message : String(error)));
    }
  };
  ...
  const handleRunCheck = async () => {
    ...
    try {
      ...
      alert('Error checking anime item: ' + (error instanceof Error ? error.message : String(error)));
    } finally {
      setIsChecking(false);
    }
  };
  ...
  const handleRunNext = async () => {
    ...
    if (currentItemIndex >= animeList.length) {
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Total items available: {animeList.length > 0 ? animeList.length : 'Not loaded'}
                </p>
              </div>
              ...
              <div className="flex flex-col justify-end gap-2">
                <Button
                  ...
                  disabled={isChecking || fetchListQuery.isLoading}
                  className="w-full"
                  variant="outline"
                >
                  {fetchListQuery.isLoading ? (
                    <>
            ...
            <div className="flex gap-2 pt-4">
              <Button
                ...
                disabled={isChecking || animeList.length === 0}
              ...
              <Button
                ...
                disabled={isChecking || animeList.length === 0 || currentItemIndex >= animeList.length}
